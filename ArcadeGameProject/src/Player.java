...
public class Player implements Findable, Temporal, Drawable {
	...
	/*
	 * uses the world's nearest dirt method to check if the player is going to
	 * dig dirt, and kills the dirt if it is dug, giving player 3 points per
	 * block
	 */
	...
	/*
	 * runs through the shield and decreases the charge if the shield is active
	 * and slowly increases the shield if inactive
	 */
	...
	/*
	 * handles the movement of the player. and checks to see if the player is
	 * inside the world. if he isn't then move him back into the world;
	 */

	public void move() {
		if (!this.isPaused) {

			switch (this.world.isInsideWorld(centerPoint, SIZE)) {
			case 'g':
				this.x += dx;
				this.y += dy;
				break;
		...
		this.centerPoint.setLocation(x + SIZE / 2, y + SIZE / 2);
	}
	...
	/*
	 * Handles the player's movement through keypresses, but only allows
	 * movement in dimension at a time
	 */
